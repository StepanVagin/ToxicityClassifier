#!/usr/bin/env python3
"""
DistilBERT-base-uncased for multilabel toxicity classification.

Implements ModelABC for compatibility with trainer.py and the rest of the pipeline.
Uses HuggingFace transformers with BCEWithLogitsLoss for multi-label classification.

DistilBERT does not use token_type_ids; the dataset and forward pass omit them.
...
class ToxicityDataset(Dataset):
    ...
    Does **not** include ``token_type_ids`` (not used by DistilBERT).
    """
    ...
    def __init__(
        ...
        if labels.ndim != 2 or labels.shape[1] != 6:
            raise ValueError(
                f"labels must have shape (n_samples, 6); got {labels.shape}"
            )
        self.texts = texts
...
class DistilBERTModel(ModelABC):
    """
    DistilBERT for multilabel toxicity classification.

    Implements ModelABC for plug-and-play with the trainer and pipeline.
    """
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        ----------
        config : dict
            Merged config (e.g. base_config + distilbert.json).

        Notes
        -----
        Optional tensor_decomposition block is read for future extension only;
        decomposition is not applied during training.
        """
        self.config = config
        ...
        print(f"DistilBERTModel: using device {self.device}")
    ...
    def train(self, train_data: Tuple, val_data: Optional[Tuple] = None) -> None:
        """
        Train DistilBERT on train_data with optional validation and early stopping.

        When validation data is provided, tracks the best weights by validation loss
        (in-memory copy of state dict) and restores them after training, including
        when early stopping fires.

        Parameters
        ----------
        train_data : (X_train, y_train)
            X_train: list of cleaned strings; y_train: (n_samples, 6) binary labels.
        ...
        self.training_history = {
            ...
            "batch_step": [],
            "batch_train_loss": [],
            "epoch_boundaries": [],  # global batch step at which each epoch ends
        }
    ...
    def _evaluate(
        ...
        f1_micro = float(
            f1_score(all_labels_arr, all_preds_arr, average="micro", zero_division=0)
        )
        f1_weighted = float(
            f1_score(all_labels_arr, all_preds_arr, average="weighted", zero_division=0)
        )
    ...
    def predict_proba(self, texts: List[str]) -> np.ndarray:
        ...
        """
        if texts is None:
            raise TypeError("texts must be a list of strings, not None")
